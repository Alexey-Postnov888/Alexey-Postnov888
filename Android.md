...
> Командный проект в рамках проектного практикума Т-Банка. Разрабатывал Android-клиент.

Стек: Kotlin, XML, Clean MVVM, Dagger, Retrofit, Kotlin Serialization, Coil, PhotoView

Мой вклад:

- Спроектировал и реализовал Android-клиент с использованием Clean MVVM и XML-разметки.
- Реализовал сетевой слой для взаимодействия с backend через REST API на базе Retrofit и Kotlin Serialization.
- Настроил внедрение зависимостей через Dagger, авторизацию запросов через кастомный Interceptor и хранение токенов через SharedPreferences.
- Реализовал навигацию с использованием вложенных графов Navigation Component.
- Добавил работу с изображениями: загрузку через Coil и интерактивный просмотр через PhotoView.
- Реализовал кастомные UI-компоненты для отображения временной шкалы (TimelineView).
...
### Дополнительный опыт (Backend и DevOps)

#### AI-ассистент в рамках Астра-стипендии (декабрь 2025 - май 2026)

> Командный проект в рамках Астра-стипендии. Разработка AI-ассистента для портала документации Astra Automation.
